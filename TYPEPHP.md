# Ship `eloquage/embed` as PHP; compile a `.so` only when you want it
...
Push/PR Pest is the pure-PHP gate (`vendor/bin/pest --coverage --min=90`, no extension). A `v*` tag runs native CI from `eloquage/typephp-builder` (`@v1`): compile, load `eloquage_embed.so` (Zend module `typephp_eloquage_embed`), and Pest with the extension enabled. Compile, load, or test failure fails the tag.
...
ELOQUAGE_TYPEPHP_IMAGE=eloquage-typephp-builder docker/typephp/build-package.sh embed
```
...
- **setup-php** — `extensions: eloquage_embed` once on PECL; otherwise Release tarball / source
- **docker-php-ext-install** — phpize tree from the Release `.tgz`
- **pecl** — `pecl install <release.tgz>` (or `eloquage_embed` after channel registration)
- **Windows** — Release `.dll` + `extension=eloquage_embed` in php.ini
